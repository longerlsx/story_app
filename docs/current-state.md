...
更新时间：2026-09-23。**ZipVoice长听发热目标仍未完成，三线程候选进入1.45×真机对照准备。** 用户原反馈来自17 Pro、1.45×、亮屏、不充电；15 Ultra问题包同类条件20分钟内电池31.6→42.7°C，用户确认明显发烫。原生推理是主要已观测CPU成本，分阶段profiling中四步Decoder占已记录推理墙钟94.8%（不是能耗占比）；未见暂停／停止后持续新生成。三线程候选保持四步、完整雷军参考及现有分段，电脑筛选与相关控制回归完成，真机2×短测出现供给停顿，尚无1.45×长听降温或节电结论。用户随后明确允许2×暂不支持，以1.45×实际发热场景验收；既定温度、电荷和连续性标准不变，只在部分语速有效必须明确条件，不能宣称全面改善。[命名候选APK](/Users/longshengxi/proj/story_app/app/build/outputs/thermal-investigation/daliamao-reader-thermal-candidate-20260923.apk)仍是待验包。上个真机窗口已恢复原包及最新数据、关闭无线调试；用户现已重新确认25–30分钟窗口并接线，已另做新备份、装回候选，待初温可比后测试。冻结标准、底层补证、结果和包身份由[发热改善计划](superpowers/plans/2026-09-23-offline-listening-thermal.md)维护；15 Ultra结果不能直接当作17 Pro验收。
