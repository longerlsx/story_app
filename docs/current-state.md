...
更新时间：2026-09-23。**已按用户要求交付“四线程／三步／1.930秒短参考”试用版并完成本轮阶段收尾，完整真机温耗目标仍未验收。** 当前源码采用该组合，音色仍来自原雷军参考首句；新增自然段首行两字缩进，单独提交。后续代理先读[本轮调查与交接](knowledge/2026-09-23-offline-tts-thermal-handoff.md)，其中汇总全部路线、有效证据、交付身份和未决项，不必重开全项目调查。两步E因用户听到滋滋声、部分字不清楚而撤回；三线程C无可靠真机温耗收益且供给停顿，也已撤回。

已测的是**三步完整参考D**：15 Ultra同书、1.45×、亮屏、不充电20分钟，电池服务33.9→40.4°C（终点实时HAL40.9°C），末5分钟skin均值43.341°C；旧四步基线分别31.6→42.7°C、45.853°C。首轮温度门槛通过，用户反馈“温热，较上次明显减轻”，实际音频连续。电荷计最后两分钟未更新，暂停后又补跳46mAh，不能虚报节电比例；至少15%的耗电改善和重复性仍未可靠证明，初温、SOC、未测室温差异保留。**这些结果不能直接当成三步短参考组合的手机结果。**

手机最后一次由代理安装的仍是[候选D APK](/Users/longshengxi/proj/story_app/app/build/outputs/thermal-investigation/daliamao-reader-thermal-candidate-d-20260923.apk)，原设置和最新阅读／听书进度已逐文件核对恢复，无线调试关闭并交还。本轮交付供用户自行覆盖安装，不自动占用手机。录音工具已固定安装，可离线复用，见[环境操作说明](knowledge/gradle-android-environment-troubleshooting.md)。用户允许2×暂不作为通过卡点；界面和实际播放倍率仍保留，其他语速、热态2×和原反馈设备17 Pro未完成本轮验收。

用户允许轻微听感损失换取较低生成成本，并选择三步短参考回去试用；这不是任意正文质量已通过的声明。[三步完整／短参考同文试听](../app/build/outputs/thermal-investigation/listening-three-steps-20260923/README.txt)在1.45×下分别约36.7／35.9秒。此前短参考在四步、两步小样本均减少生成CPU，但三步组合未测新的CPU降幅或真机能耗，不能套用百分比。原冻结门槛及逐轮过程保存在[发热改善计划](superpowers/plans/2026-09-23-offline-listening-thermal.md)，阶段结项没有改判热问题解决。

**本轮试用交付：** [三步短参考＋段首缩进APK](/Users/longshengxi/proj/story_app/app/build/outputs/thermal-investigation/daliamao-reader-3steps-shortref-indent-20260923.apk)约215.7MiB，可直接覆盖安装。缩进14项JVM、9项Android验证通过；最终1.45×实际声音、相关播放控制及模拟器覆盖安装数据保留已检查。一次“继续”控件查找失败在录屏复验未重现，根因未确定，详细证据和限制见[交付身份](knowledge/2026-09-23-offline-tts-thermal-handoff.md#本轮交付身份)。优化前四步完整参考由tag `tts-thermal-baseline-20260922-4steps-full-reference`固定，当前试用版不与其混称。
...
正文在分页和滚动两种模式中统一采用自然段首行两字宽缩进；原文已有的段首空白只在显示时规范化，同一段跨页续行不再次缩进，章节标题不缩进。缩进参与实际行测量和页底核对，不向TXT插入TAB或空格，也不改变阅读／听书保存的原文偏移。
...
当前工作区沿用sherpa-onnx 1.13.8，音源为 **ZipVoice-Distill INT8／四线程／3步＋官方 `leijun-1` 前1.930秒参考声音**（9月22日原问题包为4步／完整6.057秒）；唯一音色标识为 `zipvoice:leijun`，界面显示“雷军 · 合成音色”。这是参考声音条件下为新正文生成的合成音频，不是原讲话录音。APK内置一套模型及一个参考声音，不依赖系统TTS、联网下载或额外训练；旧 `kokoro:*` 等音色值读取时归一为新音色，书籍、进度和其他设置不因换音源清除。完整WAV仍打包，初始化时只取首句并匹配参考文字；资源版本与准备入口由[离线声音依赖](knowledge/offline-voice-dependencies.md#当前接入选择)维护。当前仅打包ARM64，候选约215MiB；旧四音色Kokoro与约443MiB包体属于9月20日版本，不是当前备用方案。
