...
更新时间：2026-09-23。**手机上的三步候选D首次真机读数达到降温门槛，耗电门槛及重复性尚待确认；不宣称发烫已解决。** 当前源码已准备两步候选E，保持四线程、完整雷军参考、原分段和实际播放倍率；用户接受两步短样本，E已构建并通过相关模拟器回归，未装手机或长测。两步与三步均低于官方所列四步配置。此前三线程候选C无可靠温耗收益且有供给停顿，已撤回。主要已观测CPU成本在原生推理，四步Decoder占记录到的推理阶段墙钟94.8%（不是能耗占比）；未见暂停／停止后持续新生成。
...
用户允许轻微听感损失换取较低生成成本，并明确优先最终组合收益，不要求精确拆分每项贡献。三步→两步小样本累计生成CPU少32.85%，各段最终时长相同，用户已反馈两步“还可以”。随后两步＋1.930秒短参考小样本比两步完整参考额外少35.18%生成CPU，但两句时长分别约−7%／＋7%，未过原定节奏标准，不能将其记为无退化。用户尚无法判断组合是否可接受，要求更长且含对话、数字的四步／两步／组合对照；已准备[三份35–37秒试听与说明](../app/build/outputs/thermal-investigation/listening-comparison-20260923/README.txt)，统一1.45×和平均音量，等待用户及其他试听者反馈后再选最终候选。未改手机，短参考未进入App，不把电脑CPU降幅当作节电比例。按用户明确要求，试听包备妥后暂停目标。用户另允许过期验收代码删除后释放额度，预算明细见计划，不修改跨任务稳定规则。
