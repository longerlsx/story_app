...
更新时间：2026-09-23。**ZipVoice长听发热尚未解决，三线程候选已撤回。** 15 Ultra亮屏、不充电、1.45×的20分钟实测：三线程电池终温43.3°C／电荷减少451mAh，四线程基线42.7°C／456mAh；起温和SOC不同，温度未达目标，5mAh差异不足以证明节电。三线程实录还出现多次长静音，已确认其中至少两处是供给停顿。原生推理为主要已观测CPU成本；四步Decoder占记录到的推理阶段墙钟94.8%（不是能耗占比），未见暂停／停止后持续新生成。默认已恢复四线程，手机恢复原包及本次新备份的数据、关闭无线调试并交还。用户允许2×暂不支持，以1.45×验收；允许记录实际初温直接测量，温度／耗电目标不降低，部分语速有效须明确范围。电脑侧短参考和合块试验未形成可接受主候选；新增三步生成同文样本有约25%累计CPU下降、时长近似，但低于官方所列四步配置，用户听完对照认为基本相当并允许继续验证。四线程／三步／完整参考候选D已构建，现有三项模拟器听书回归及1.45×单句实声通过；内置录音会省略部分空闲，不能据其宣布跨段连续性通过。候选尚无手机温度／耗电结论。16:13约定窗口因连续录音工具下载未恢复而未执行；未安装D或写入测试设置，已重新打开原阅读页并交还手机。录音工具现已固定安装并通过模拟器实录预检；用户重新授权窗口后，D于16:43:31开始15 Ultra的1.45×亮屏不充电20分钟测量，尚待结果。对照、限制、底层成本与历史候选身份由[发热改善计划](superpowers/plans/2026-09-23-offline-listening-thermal.md)维护；原反馈来自17 Pro，尚不能用15 Ultra结果替代其验收。
