...
更新时间：2026-09-23。**手机上的三步候选D首次真机读数达到降温门槛，耗电门槛及重复性尚待确认；不宣称发烫已解决。** 用户听较长样本后反馈两步部分字有“电视滋滋声”、听不清，因此撤回两步候选E，源码恢复D的四线程／三步／完整参考，手机仍是D。E的历史构建和模拟器通过不代表音质可接受，E APK仅保留调查身份。此前三线程候选C无可靠温耗收益且有供给停顿，也已撤回。主要已观测CPU成本在原生推理，四步Decoder占记录到的推理阶段墙钟94.8%（不是能耗占比）；未见暂停／停止后持续新生成。
...
用户允许轻微听感损失换取较低生成成本，并明确优先最终组合收益，不要求精确拆分每项贡献。较长试听暴露两步的清晰度问题后，用户要求改听“三步＋短参考”，认为短参考本身影响似乎较小，尚未接受新组合。已提供[三步完整／短参考同文试听](../app/build/outputs/thermal-investigation/listening-three-steps-20260923/README.txt)，沿用上一轮正文、分段、1.45×及相同整体音量，分别约36.7／35.9秒；按用户要求备妥后暂停，不自动安装或安排手机测试。此前短参考在四步、两步小样本均减少生成CPU，但三步组合尚未测定收益，不能套用其百分比或当作手机节电结论。短参考未进入App。用户另允许过期验收代码删除后释放额度，预算明细见计划，不修改跨任务稳定规则。
