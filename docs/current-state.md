...
更新时间：2026-09-23。**三步候选首次真机读数达到降温门槛，耗电门槛及重复性尚待确认；不宣称发烫已解决。** 当前候选D保持四线程、完整雷军参考、原分段和实际播放倍率，只将生成四步改为三步；低于官方所列四步配置，用户已听同文短样本并接受继续验证。此前三线程候选C无可靠温耗收益且有供给停顿，已撤回。主要已观测CPU成本在原生推理，四步Decoder占记录到的推理阶段墙钟94.8%（不是能耗占比）；未见暂停／停止后持续新生成。
