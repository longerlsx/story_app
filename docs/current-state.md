...
更新时间：2026-09-23。**ZipVoice长听发热改善已完成首轮候选准备，按用户要求等待约定真机窗口；目标未完成。** 用户原反馈来自17 Pro、1.45×、亮屏、不充电，具体多久不确定；17 Pro暂不在场。15 Ultra问题包在同类条件20分钟内电池31.6→42.7°C，用户确认外壳明显发烫；主要CPU时间在原生推理库。1.45×有一次跨短段长静音，热态2×出现多次供给停顿；暂停／停止后未见持续失控计算。横向研究及微样本未支持自旋调整或直接加长正文。三线程候选保持四步、完整雷军参考及现有分段，两轮模拟器累计CPU均下降，三项实际控制回归与有边界独立检查完成，短2×录音没有检出≥0.6秒供给静音；首声存在波动，真机降温、耗电与热态2×均未验收。已保留[可覆盖安装的候选APK](/Users/longshengxi/proj/story_app/app/build/outputs/thermal-investigation/daliamao-reader-thermal-candidate-20260923.apk)，尚未安装手机。手机已归还，必须重新约定时长并确认开始才继续真机测试，且先备份当时最新数据。冻结标准、结果、包身份和下一步由[发热改善计划](superpowers/plans/2026-09-23-offline-listening-thermal.md)维护；15 Ultra结果不能直接当作17 Pro验收。
