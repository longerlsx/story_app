...
- If UIAutomator reader tests pass alone but fail in mixed groups, logcat says a non-clickable Compose semantics node was clicked, or reset helpers delete files but leave app-process state alive, start with `BUG-2026-015`; the confirmed fix is center-coordinate clicks plus helpers that leave chrome and in-memory repositories in a known state.
